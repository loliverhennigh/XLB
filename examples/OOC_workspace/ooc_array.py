...
class OOCArray:
    ...
    def __init__(
        ...
        self.compute_tiles = []
        self.compute_wait_events = []
        self.compute_copy_streams = []
        for i in range(self.nr_compute_tiles):
            # Make compute tile
            compute_tile = self.ComputeTile(self.tile_shape, self.dtype, self.padding)
            self.compute_tiles.append(compute_tile)

            # Make wait event
            self.compute_wait_events.append(None)

            # Make copy stream
            self.compute_copy_streams.append(cp.cuda.Stream(non_blocking=True))
    ...
    def managed_compute_tiles(self, tile_index):
        ...
        if tile_index in self.compute_tile_map:
            ...
            stream.wait_event(self.compute_wait_events[compute_tile_index])
            return compute_tile
        ...
        # Start asynchronous copying of the tile to the compute array
            ...
            for compute_tile_index in range(self.nr_compute_tiles):
                with self.compute_copy_streams[compute_tile_index] as stream:
                    ...
                    # Set the wait event
                    self.compute_wait_events[compute_tile_index] = stream.record()
            ...
            # Wait for the compute tile to be copied
            stream = cp.cuda.get_current_stream()
            stream.wait_event(self.compute_wait_events[0])
    ...
    def set_tile(self, compute_array, tile_index):
        ...
        compute_tile = self.compute_tiles[self.compute_tile_map[tile_index]]
