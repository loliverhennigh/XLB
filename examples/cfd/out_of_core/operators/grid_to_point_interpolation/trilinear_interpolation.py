...
class TrilinearInterpolation(Operator):
    """
    Operator for trilinear interpolation from a 4D grid to points in 3D space.
    ...
    def __call__(
        ...
        """
        Interpolate values from a 4D grid to points in 3D space.
        ...
        point_values : wp.array2d(dtype=float), optional
            Output array with shape (q, num_points). If None, will be created.
        ...
        if point_values is None:
            point_values = wp.zeros((grid.shape[0], points.shape[0]), dtype=float)
