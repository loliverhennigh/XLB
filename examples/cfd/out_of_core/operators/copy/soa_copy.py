...
class SOACopy(Operator):
    """
    SOACopy is an operator for copying data from a source array to a destination array.
    It supports both 2D and 3D arrays using the Structure of Arrays (SoA) format.
    """
    ...
    def __call__(
        ...
        kernel = (
            self.soa_copy_3d if len(dest.shape) == 4 else
            self.soa_copy_2d if len(dest.shape) == 3 else
            self.soa_copy_1d
        )
        wp.launch(
            kernel,
