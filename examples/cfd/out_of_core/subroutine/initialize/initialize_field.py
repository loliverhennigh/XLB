...
class InitializeFieldSubroutine(AMRGridSubroutine):
                ...
                if block.pid == amr_grid.pid:
                    ...
                    for ghost_block, ghost_boxes in block.local_ghost_boxes.items():
                        field_ghost[ghost_block] = self.memory_pools[stream_idx].get(
                            ghost_boxes[field_name].data_shape,
                            ghost_boxes[field_name].dtype
                        )
                    ...
                    for ghost_block, ghost_boxes in block.local_ghost_boxes.items():
                        ...
                        slice_start = (ghost_boxes[field_name].offset - block.boxes[field_name].offset) // 2 ** block.amr_level
