...
class CompressedGPUTile(CompressedTile):
    ...
    def to_array(self, array):
        ...
        self.dense_gpu_tile._array = _decode(self._array[:self._array_bytes], self.dense_gpu_tile._array, self._array_codec)
        ...
        for pad_ind in self.pad_ind:
            if pad_ind not in self._padding_codec:
                ...
                self._padding_codec[pad_ind].decompression_config = (
                    self._padding_codec[pad_ind]._manager.configure_decompression_with_compressed_buffer(asarray(self._padding[pad_ind][:self._padding_bytes[pad_ind]]))
                )
            self.dense_gpu_tile._padding[pad_ind] = _decode(self._padding[pad_ind][:self._padding_bytes[pad_ind]], self.dense_gpu_tile._padding[pad_ind], self._padding_codec[pad_ind])
    ...
    def from_array(self, array):
        ...
        for pad_ind in self.pad_ind:
            ...
            self._padding_bytes[pad_ind] = _encode(array[self._slice_array_to_padding[pad_ind]], self._padding[pad_ind], self._padding_codec[pad_ind])
