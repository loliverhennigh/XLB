...
class LBMBase(object):
    ...
    ----------
        ...
        boundary_conditions (list): A list of boundary conditions to be applied to the simulation.
    ...
    def __init__(self, **kwargs):
        ...
        if self.checkpointRate > 0:
            ...
            self.mngr = orb.CheckpointManager(
                self.checkpointDir, orb.PyTreeCheckpointer(), options=mngr_options
            )
        else:
            self.mngr = None
        ...
        if self.dim == 2:
            ...
            self.compute_bitmask = jit(
                shard_map(
                    self.compute_bitmask_m,
                    mesh=self.mesh,
                    in_specs=P("x", None, None),
                    out_specs=P("x", None, None),
                    check_rep=False,
    ...
    def _create_boundary_data(self):
        ...
        print(
            "Time to create the local bitmasks and normal arrays:", time.time() - start
        )
    ...
    def create_grid_connectivity_bitmask(self, solid_halo_voxels):
        ...
        hw_y = hw_z = 1
        if self.dim == 2:
            connectivity_bitmask = self.distributed_array_init(
                (self.nx + 2 * hw_x, self.ny + 2 * hw_y, self.lattice.q),
                jnp.bool_,
                init_val=True,
            )
            connectivity_bitmask = connectivity_bitmask.at[
    ...
    def compute_bitmask_m(self, b):
        ...
        left_comm, right_comm = (
            b[:1, ..., self.lattice.right_indices],
            b[-1:, ..., self.lattice.left_indices],
        )
    ...
    def compute_bitmask_p(self, b):
        ...
        return vmap(compute_bitmask_i, in_axes=(None, 0), out_axes=-1)(
            b, self.lattice.i_s
        )
